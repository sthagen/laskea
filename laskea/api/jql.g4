...
jql_stmt
 : PAR_OPEN* expr ( ( K_AND | K_OR) PAR_OPEN* expr PAR_CLOSE* )* PAR_CLOSE* ordering_term?
 ;

expr
 : K_NOT* ( field | literal_value ) operator PAR_OPEN* ( literal_value | literal_list | FUNCTION | dates ) (compare_dates)? PAR_CLOSE*
 ;
...
IDENTIFIER
 ...
 | '`' (~'`' | '``')* '`'
 | '[' ~']'* ']'
 | [a-zA-Z_] [a-zA-Z_0-9.\-]* // This should be all characters in the use case
 | '-'
 | [A-Z]+ '-' [0-9]+ // identifier/key pattern
 ;
